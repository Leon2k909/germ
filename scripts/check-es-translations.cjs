#!/usr/bin/env node
/**
 * The same guard as the Russian, Polish and Italian checks, for the Spanish
 * course and its German table — the last of the seven country courses to get
 * one.
 *
 * The table is keyed on the course's own Spanish text, and it fails the same
 ...
 * than demanded.
 *
 * THE QUIZZES ARE PART OF THE TABLE, as in the three tables before it.
 *
 * HOW A PASTE IS CAUGHT HERE. Like Italian, Spanish shares its alphabet with
 * German, so the function words do the work: no German sentence holds "que",
 * "del", "para" or "los". One exception had to be left out of the list —
 * "es", which is the Spanish verb and the German pronoun, and would have
 * flagged half the table on its own.
 */
...
const built = esbuild.buildSync({
  stdin: {
    ...
    resolveDir: root,
    sourcefile: "es-translations-entry.ts",
  },
...
const compiled = new Module("es-translations-check", module);
compiled.filename = path.join(root, ".es-translations-check.cjs");
compiled.paths = Module._nodeModulePaths(root);
compiled._compile(built.outputFiles[0].text, compiled.filename);
const { vivirEnEspanaCourse: course, VIVIR_EN_ESPANA_DE: table } = compiled.exports;

// Every Spanish string the reader can meet, split so the coverage line can say
...
const entryLines = fs
  .readFileSync(path.join(root, "src/lib/vivirEnEspanaTranslationsDe.ts"), "utf8")
  .split(/\r?\n/)
...
// ── a German value that is still Spanish ───────────────────────────────────
const SPANISH_WORDS = new Set([
  "el", "la", "los", "las", "un", "una", "unos", "unas", "del", "al", "de", "que", "con", "por",
  "para", "como", "más", "son", "está", "están", "ser", "este", "esta", "estos", "estas", "su",
  "sus", "se", "no", "y", "pero", "cuando", "donde", "porque", "entre", "cada", "todos", "todas",
  "desde", "hasta", "muy", "también", "aunque", "puede", "pueden", "debe", "deben", "hay", "fue",
  "eran", "sobre", "según", "sin", "sino", "cuyo", "cual", "quien",
]);
const stillSpanish = Object.entries(table).filter(([, value]) => {
  const words = String(value).toLowerCase().split(/[^\p{L}áéíóúñü]+/u).filter(Boolean);
  if (words.length < 6) return false;
  const spanish = words.filter((word) => SPANISH_WORDS.has(word)).length;
  return spanish / words.length > 1 / 6;
});
if (stillSpanish.length) {
  failures.push(
    `${stillSpanish.length} value(s) still read as Spanish rather than German: `
    + stillSpanish.slice(0, 4).map(([key]) => JSON.stringify(key.slice(0, 60))).join(", ")
  );
}

// A sentence identical to its Spanish is the same fault seen from the other
...
if (identical.length) {
  failures.push(
    `${identical.length} sentence(s) are identical to their Spanish: `
    + identical.slice(0, 4).map(([key]) => JSON.stringify(key.slice(0, 60))).join(", ")
  );
}

// ── the words the course is about have to survive ──────────────────────────
// A translation that turned the Cortes into a parliament and a comunidad
// autónoma into a province would teach the reader words nobody in the country
// uses. A handful may legitimately reword; a majority losing the term is the
// failure worth catching.
const KEEP = [
  { spanish: "Constitución", german: "Verfassung" },
  { spanish: "Cortes", german: "Cortes" },
  { spanish: "Gobierno", german: "Regierung" },
  { spanish: "autónoma", german: "autonome" },
  { spanish: "Rey", german: "König" },
];
for (const { spanish, german } of KEEP) {
  const withTerm = Object.entries(table).filter(([key]) => key.includes(spanish));
  ...
  if (withTerm.length >= 4 && dropped.length > withTerm.length / 2) {
    failures.push(
      ...
      + `"${spanish}" — that is the word the country uses and it should stay in the sentence`
    );
  }
}
...
assert.ok(
  /id: "de"[^}]*from: \[[^\]]*"es"/.test(lib),
  "German must be registered as translating FROM Spanish, or it is never offered beside this course"
);
assert.ok(
  /de: \{[\s\S]{0,400}?VIVIR_EN_ESPANA_DE/.test(lib),
...
// ── and now every country course has one ───────────────────────────────────
// The point of finishing is that no course is left where the picker hides
// itself. If a new pack arrives without a table, this line says so.
const CONTENT_LANGUAGES = ["en", "de", "fr", "pl", "it", "es", "ru"];
const packs = fs.readFileSync(path.join(root, "src/lib/countryPacks.ts"), "utf8");
const shipped = [...packs.matchAll(/contentLang: "(\w+)"/g)].map((match) => match[1]);
for (const language of shipped) {
  if (!CONTENT_LANGUAGES.includes(language)) {
    failures.push(`a pack is written in "${language}", which this check does not know about`);
  }
}

if (failures.length) {
  console.error("FAIL check-es-translations");
...
console.log(
  `check-es-translations: ${doneProse} of ${prose.size} cards and headings (${pct(doneProse, prose.size)}%) `
  + `and ${doneQuiz} of ${quiz.size} question strings (${pct(doneQuiz, quiz.size)}%) have German, `
  + "every key matches real course text, none is written twice, and the picker offers it beside the Spanish course"
);
process.exit(0);
