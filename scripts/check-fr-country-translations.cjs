#!/usr/bin/env node
/**
 * The French tables for the country courses that are not written in French.
 *
 * Life in the UK and Leben in Deutschland have gates of their own —
 * check-uk-translations and check-de-translations — because those two files
 * carry several targets at once. The courses here have a French table and
 * nothing else checking it, so they share this file, the way
 * check-en-translations covers three: a fourth joins by adding four lines to
 * COURSES rather than by copying two hundred.
 *
 ...
 *
 *   - a table never spread into TRANSLATIONS.fr is looked up by nobody;
 *   - a source language missing from French's `from` list means the picker
 *     never offers French beside that course, so the table is unreachable
...
/**
 * One row per French table for a country course.
 *
 * `sourceLetters` are the letters that language has and French does not, so a
 * value still written in it can be recognised. Proper names are exempt from
 * that test where the two alphabets are shared: Kraków and Łódź are correct
 * inside a French sentence, and only a LOWERCASE word carrying those letters
 * is a line nobody translated. For a Cyrillic source there is no such
 * exemption — French keeps no Cyrillic at all, names included.
 */
const COURSES = [
  {
    ...
    table: { module: "zycieWPolsceTranslationsFr", symbol: "ZYCIE_W_POLSCE_FR" },
    sourceLetters: /[ąćęłńóśźż]/,
    // Only terms with NO French equivalent belong here. Konstytucja and
    // Parlament do have one and should be translated; demanding that they
    // survive would ask for worse French. What is left is the vocabulary a
...
const MERGED = [
  { label: "Life in the UK", module: "lifeInTheUkTranslationsFr", symbol: "LIFE_IN_THE_UK_FR" },
  { label: "Leben in Deutschland", module: "lebenInDeutschlandTranslationsFr", symbol: "LEBEN_IN_DEUTSCHLAND_FR" },
];
...
const built = esbuild.buildSync({
  stdin: { contents: imports, resolveDir: root, sourcefile: "fr-country-translations-entry.ts" },
...
const compiled = new Module("fr-country-translations-check", module);
compiled.filename = path.join(root, ".fr-country-translations-check.cjs");
...
for (const entry of COURSES) {
  ...
  const untranslated = Object.entries(table).filter(([, value]) => {
    ...
    const foreign = words.filter((word) => {
      ...
      // Cyrillic never belongs in a French sentence, name or not.
  ...
  for (const key of Object.keys(table)) {
    if (seen.has(key)) {
      failures.push(
        ...
        + "spread into TRANSLATIONS.fr — one of the two translations is lost with no error anywhere"
      );
    } else {
...
const from = /id: "fr"[^}]*from: \[([^\]]*)\]/.exec(lib);
assert.ok(from, "French has no from list in TRANSLATION_LANGUAGES");
for (const entry of COURSES) {
  assert.ok(
    new RegExp(`fr: \\{[^}]*${entry.table.symbol}`).test(lib),
    `${entry.table.symbol} is not registered in TRANSLATIONS, so nothing would ever look it up`
  );
  assert.ok(
    from[1].includes(`"${entry.source}"`),
    `French must be registered as translating FROM "${entry.source}", or it is never offered beside ${entry.label}`
  );
}

if (failures.length) {
  console.error("FAIL check-fr-country-translations");
  failures.forEach((line) => console.error("  " + line));
  process.exit(1);
}

console.log("check-fr-country-translations: every key matches real course text, no table collides with another,");
summaries.forEach((line) => console.log(line));
console.log("  and every table is spread into TRANSLATIONS.fr with its source language in the from list");
process.exit(0);
